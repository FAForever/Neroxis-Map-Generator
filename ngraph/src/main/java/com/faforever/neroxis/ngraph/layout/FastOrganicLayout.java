...
public class FastOrganicLayout extends GraphLayout {
    ...
    public void execute(ICell parent) {
        ...
        List<ICell> tmp = new ArrayList<>(vertices.length);

        for (ICell o : vertices) {
            if (!isVertexIgnored(o)) {
                tmp.add(o);
            }
        }

        vertexArray = tmp.toArray(new ICell[0]);
        ...
        try {
            for (int i = 0; i < n; i++) {
                ...
                ICell[] edges = graph.getConnections(vertexArray[i], parent);
