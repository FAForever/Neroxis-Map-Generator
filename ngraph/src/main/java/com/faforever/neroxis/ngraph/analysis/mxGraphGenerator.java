...
import com.faforever.neroxis.ngraph.costfunction.mxDoubleValCostFunction;
import com.faforever.neroxis.ngraph.generatorfunction.mxGeneratorFunction;
import com.faforever.neroxis.ngraph.generatorfunction.mxGeneratorRandomFunction;
		...
		for (int i = 0; i < numBranches; i++) {
			for (int j = 0; j < numVerticesInBranch; j++) {
				...
				if (isBranchSizeEven && j == middleIndex - 1) {
					...
					if (j + 1 < middleIndex) {
						//before middle
						currRatio = ((i - 1f / Math.pow(currSize, 0.25) + 0.00000000000015f * Math.pow(currSize, 4)) / (double) numVertexesInPerimeter);
					} else {
						//after middle
						currRatio = ((i + 1f / Math.pow(currSize, 0.25) - 0.00000000000015f * Math.pow(currSize, 4)) / (double) numVertexesInPerimeter);
					}
