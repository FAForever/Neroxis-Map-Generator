...
import com.faforever.neroxis.ngraph.costfunction.mxCostFunction;
...
import com.faforever.neroxis.ngraph.view.mxGraph.mxICellVisitor;
...
public class mxGraphStructure {
	 ...
	 */
	private static final String basicVertexStyleString = "ellipse;strokeColor=black;fillColor=orange;gradientColor=none";

	/**
	 * The default style for edges
	 */
	private static final String basicEdgeStyleString = "strokeColor=red;noEdgeStyle=1;";

	private static final String basicArrowStyleString = "endArrow=block;";
	...
	public static boolean isTree(mxAnalysisGraph aGraph) {
		return isConnected(aGraph) && !isCyclicUndirected(aGraph) && isSimple(aGraph);
	}
	...
	public static boolean isCutVertex(mxAnalysisGraph aGraph, Object vertex) {
		...
		if (aGraph.getEdges(vertex, null, true, true, false, true).length >= 2) {
			...
			Object newVertex = getVertexWithValue(aGraphCopy, (int) aGraph.getGenerator().getCostFunction().getCost(new mxCellState(graph.getView(), vertex, null)));
			...
			return newComponents.length > oldComponents.length;
		}
	...
	public static boolean isBiconnected(mxAnalysisGraph aGraph) {
		...
		return getCutVertices(aGraph).length == 0 && edgeCount >= 1;
	}
