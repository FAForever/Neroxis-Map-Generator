...
import com.faforever.neroxis.ngraph.view.mxPerimeter.mxPerimeterFunction;
...
public class mxGraphView extends mxEventSource {
	 ...
	 *
	 */
	private static final mxPoint EMPTY_POINT = new mxPoint();
	...
	public void validate() {
		mxRectangle graphBounds = getBoundingBox(validateCellState(validateCell((currentRoot != null) ? currentRoot : graph.getModel().getRoot())));
	...
	public void updateLabelBounds(mxCellState state) {
		...
		if (overflow.equals("fill")) {
			...
			state.setLabelBounds(mxUtils.getLabelPaintBounds(state.getLabel(), style, graph.isHtmlLabel(cell), state.getAbsoluteOffset(), vertexBounds, scale, graph.getModel().isEdge(cell)));
	...
	public void updateFloatingTerminalPoint(mxCellState edge, mxCellState start, mxCellState end, boolean source) {
		...
		border += mxUtils.getDouble(edge.getStyle(), (source) ? mxConstants.STYLE_SOURCE_PERIMETER_SPACING : mxConstants.STYLE_TARGET_PERIMETER_SPACING);
