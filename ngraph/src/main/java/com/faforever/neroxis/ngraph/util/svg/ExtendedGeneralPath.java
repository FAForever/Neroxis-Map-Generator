 ...
 */
package com.faforever.neroxis.ngraph.util.svg;
...
public class ExtendedGeneralPath implements Shape, Cloneable {
	...
	public synchronized void arcTo(float rx, float ry, float angle, boolean largeArcFlag, boolean sweepFlag, float x, float y) {
		...
		makeRoom(7);
		types[numSeg++] = com.faforever.neroxis.ngraph.util.svg.ExtendedPathIterator.SEG_ARCTO;
		values[numVals++] = rx;
		values[numVals++] = ry;
	 ...
	 * Delegates to the enclosed <code>GeneralPath</code>.
	 */
	public void append(com.faforever.neroxis.ngraph.util.svg.ExtendedPathIterator epi, boolean connect) {
		...
		while (!epi.isDone()) {
			...
			switch (type) {
				...
				case PathIterator.SEG_CUBICTO:
					...
					break;
				case com.faforever.neroxis.ngraph.util.svg.ExtendedPathIterator.SEG_ARCTO:
	 ...
	 * Delegates to the enclosed <code>GeneralPath</code>.
	 */
	public com.faforever.neroxis.ngraph.util.svg.ExtendedPathIterator getExtendedPathIterator() {
		return new EPI();
	}
	...
	class EPI implements com.faforever.neroxis.ngraph.util.svg.ExtendedPathIterator {
		int segNum = 0;
