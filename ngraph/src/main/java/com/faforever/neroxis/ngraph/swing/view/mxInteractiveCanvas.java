...
import com.faforever.neroxis.ngraph.shape.mxBasicShape;
...
public class mxInteractiveCanvas extends mxGraphics2DCanvas {
	...
	public boolean hitSwimlaneContent(mxGraphComponent graphComponent, mxCellState swimlane, int x, int y) {
		if (swimlane != null) {
			int start = (int) Math.max(2, Math.round(mxUtils.getInt(swimlane.getStyle(), mxConstants.STYLE_STARTSIZE, mxConstants.DEFAULT_STARTSIZE) * graphComponent.getGraph().getView().getScale()));
