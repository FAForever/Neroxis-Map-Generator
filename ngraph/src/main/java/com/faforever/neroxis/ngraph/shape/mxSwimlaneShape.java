...
import com.faforever.neroxis.ngraph.canvas.mxGraphicsCanvas2D;
...
public class mxSwimlaneShape extends mxBasicShape {
    ...
    protected double getTitleSize(mxGraphics2DCanvas canvas, mxCellState state) {
        return Math.max(0, mxUtils.getFloat(state.getStyle(), mxConstants.STYLE_STARTSIZE, mxConstants.DEFAULT_STARTSIZE) * canvas.getScale());
    }
    ...
    protected double getArcSize(mxCellState state, double start) {
        double f = mxUtils.getDouble(state.getStyle(), mxConstants.STYLE_ARCSIZE, mxConstants.RECTANGLE_ROUNDING_FACTOR * 100) / 100;
    ...
    protected mxGraphicsCanvas2D configureCanvas(mxGraphics2DCanvas canvas, mxCellState state, mxGraphicsCanvas2D c) {
        ...
        if (!mxConstants.NONE.equals(fill) && !mxConstants.NONE.equals(gradient)) {
            ...
            c.setGradient(fill, gradient, b.getX(), b.getY(), b.getWidth(), b.getHeight(), mxUtils.getString(state.getStyle(), mxConstants.STYLE_GRADIENT_DIRECTION, mxConstants.DIRECTION_NORTH), 1, 1);
        } else {
            c.setFillColor(fill);
        }
