...
import com.faforever.neroxis.ngraph.util.mxLightweightLabel;
...
public class mxGraphicsCanvas2D implements mxICanvas2D {
	...
	protected String createHtmlDocument(String text, String align, String valign, int w, int h, boolean wrap, String overflow, boolean clip) {
		...
		css.append("line-height:" + ((mxConstants.ABSOLUTE_LINE_HEIGHT) ? Math.round(state.fontSize * mxConstants.LINE_HEIGHT) + " " + HTML_UNIT : mxConstants.LINE_HEIGHT) + ";");
		...
		if (state.fontColor != null) {
			...
			for (int i = 0; i < lines.length; i++) {
				...
				if (!lines[i].isEmpty()) {
					...
					boolean isStrikethrough = (state.fontStyle & mxConstants.FONT_STRIKETHROUGH) == mxConstants.FONT_STRIKETHROUGH;
