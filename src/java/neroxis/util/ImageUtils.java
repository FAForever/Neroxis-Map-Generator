...
public class ImageUtils {
    ...
    public static void writePNGFromMasks(FloatMask redMask, FloatMask greenMask, FloatMask blueMask, float scaleMultiplier, String path) throws IOException {
        ...
        for (int x = 0; x < size; x++) {
            for (int y = 0; y < size; y++) {
                ...
                index += 3;
                byteArray[index - 2] = redByte;
                byteArray[index - 1] = greenByte;
    ...
    public static void writeAutomaticallyScaledPNGFromMasks(FloatMask redMask, FloatMask greenMask, FloatMask blueMask, String path) throws IOException {
        int size = redMask.getSize();
        if (size != greenMask.getSize() || size != blueMask.getSize()) {
            throw new IllegalArgumentException("Masks not the same size: redMask is " + redMask.getSize() +
                    ", greenMask is " + greenMask.getSize() + " and blueMask is " + blueMask.getSize());
    ...
    public static void writeAutomaticallyScaledPNGFromMask(FloatMask mask, String path) throws IOException {
