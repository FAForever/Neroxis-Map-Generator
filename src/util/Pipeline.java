...
public class Pipeline {
	...
	public static void stop() {
        pipeline.stream().map(Entry::getFuture).forEach(CompletableFuture::join);
...
//		}
	...
	public static CompletableFuture<List<ConcurrentMask>> getDependencyFuture(List<Entry> dependencyList, ConcurrentMask requestingMask) {
		...
		CompletableFuture<?>[] futures = ((CompletableFuture<?>[]) dependencyList.stream().map(Entry::getFuture).toArray(CompletableFuture<?>[]::new));
...
//		IntStream.range(0, 200).forEach(aVoid -> {
//			boolean[][] b = new boolean[513][513];
//			for(int x = 0;x < 513;x++) {
//				for(int y = 0;y < 513;y++) {
//					b[x][y] = references.get(references.size() - 1)[x][y];
//				}
//			}
//			references.add(b);
//		});
...
//	}
	...
	public static class Entry {
		...
		private Set<Entry> dependants = new HashSet<>();

		private List<ConcurrentMask> maskBackups = new ArrayList<>();
